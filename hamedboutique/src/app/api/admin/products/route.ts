...
export async function POST(request: NextRequest) {
  ...
  const { title, price, image, images, description, category, discount, inventory } = body;
  if (!title || !price) {
    return NextResponse.json({ error: 'عنوان و قیمت الزامی است' }, { status: 400 });
  }
  await initDb();
  const result = await addProduct({ title, price, image, images, description, category, discount, inventory });
